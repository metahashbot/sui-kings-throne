...
{
	[Serializable]
	[TypeInfoBox("增幅反映——【灵威】 = 灵+光，增幅灵")]
	public class Buff_灵威__ElementSpiritOppression : BaseRPBuff, I_BuffAsElementSecond,
		I_BuffAsElementAmplification, I_BuffContainLoopVFX
	{
#region 配置

		[LabelText("增加【灵】伤害的比率(50表示增加50%的伤害)")]
		[TitleGroup("===基本配置===")]
		public float SoulDamageBonusRatio = 50f;
		...
		private void _ABC_CheckIfBonusElementDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (dar.DamageType != DamageTypeEnum.LingNengLing_灵能灵)
			{
				return;
			}
			dar.CP_DamageAmount_BPart.MultiplyPart += SoulDamageBonusRatio * CurrentStackCount / 100f;
		}
		...
		{
			return $"当前层数{CurrentStackCount},已增伤{SoulDamageBonusRatio * CurrentStackCount}";
		}
