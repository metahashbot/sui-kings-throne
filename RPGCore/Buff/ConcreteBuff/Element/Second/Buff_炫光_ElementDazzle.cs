...
{
	[TypeInfoBox("增幅反应。炫光【光+水】\n")]
	[Serializable]
	public class Buff_炫光_ElementDazzle : BaseRPBuff, I_BuffAsElementSecond , I_BuffAsElementAmplification,
		I_BuffContainLoopVFX
	{
		
#region 配置

		[LabelText("增加【光】伤害的比率(50表示增加50%的伤害)")]
		[TitleGroup("===基本配置===")]
		public float LightDamageBonusRatio = 50f;
			...
			I_RP_Buff_ObjectCanApplyBuff selfReceiveFrom)
		{
			...
			CurrentStackCount = 1;
			parent.ReceiveBuff_GetRelatedActionBus().RegisterAction(ActionBus_ActionTypeEnum.L_DamageProcess_Receiver_BeforeFrontMul_对接收方将要进行前乘额外算区计算,
		...
		private void _ABC_CheckIfBonusElementDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (dar.DamageType != DamageTypeEnum.YuanNengGuang_源能光)
			{
				return;
			}
			dar.CP_DamageAmount_BPart.MultiplyPart += LightDamageBonusRatio * CurrentStackCount / 100f;
		...
		{
			return $"当前层数{CurrentStackCount},已增伤{LightDamageBonusRatio * CurrentStackCount}";
		}
