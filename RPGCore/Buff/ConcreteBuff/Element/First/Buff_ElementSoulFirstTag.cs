...
namespace RPGCore.Buff.ConcreteBuff.Element.First
{
	...
	[Serializable]
	public class Buff_ElementSoulFirstTag : FirstElementTagBuff
	{

		private new void _ABC_ProcessElementAffection(DS_ActionBusArguGroup ds)
		{
			...
			switch (targetType)
			{
				...
				case RolePlay_BuffTypeEnum.ElementFirstWindTag_Feng_一级风标签:
					Parent_SelfBelongToObject.ReceiveBuff_TryApplyBuff(
						RolePlay_BuffTypeEnum.ElementSecondWindSuppress_FengYa_二级风压,
						Parent_SelfBelongToObject as I_RP_Buff_ObjectCanApplyBuff,
						Parent_SelfBelongToObject,
						blpList);
					break;
				case RolePlay_BuffTypeEnum.ElementSecondOverload_ChaoZai_二级超载:
					case RolePlay_BuffTypeEnum.ElementSecondWindSuppress_FengYa_二级风压:
					ModifyStack(true, -1);
					break;
			}
		}
