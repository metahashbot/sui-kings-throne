using System;
using System.Collections.Generic;
using Global.ActionBus;
using RPGCore.DataEntry;
using RPGCore.UtilityDataStructure;
using Sirenix.OdinInspector;
namespace RPGCore.Buff.ConcreteBuff
{
	[Serializable]
	[TypeInfoBox("同步SP最大值时变动到当前SP，并且为当前值设置上界为最大值\n")]
	
	public class Buff_SyncSPPartialOnMaxChanged : BaseRPBuff
	{

		private Float_RPDataEntry _entry_MaxSP;
		private Float_RPDataEntry _entry_CurrentSP;

		public override DS_ActionBusArguGroup OnBuffInitialized(List<BaseBuffLogicPassingComponent> blps)
		{
			var ds = base.OnBuffInitialized(blps);
			Parent_SelfBelongToObject.ReceiveBuff_GetRelatedActionBus()
				.RegisterAction(ActionBus_ActionTypeEnum.L_DataEntry_OnDataEntryValueChanged_数据项的值被改变, _ABC_SyncData);
			_entry_MaxSP =
				Parent_SelfBelongToObject.ReceiveBuff_GetFloatDataEntry(RP_DataEntry_EnumType.SPMax_最大SP);
			_entry_CurrentSP =
				Parent_SelfBelongToObject.ReceiveBuff_GetFloatPresentValue(RP_DataEntry_EnumType.CurrentSP_当前SP);
			_entry_CurrentSP.SetUpperBound(_entry_MaxSP.CurrentValue);
			return ds;
		}


		public override DS_ActionBusArguGroup OnBuffPreRemove()
		{
			Parent_SelfBelongToObject.ReceiveBuff_GetRelatedActionBus()
				.RemoveAction(ActionBus_ActionTypeEnum.L_DataEntry_OnDataEntryValueChanged_数据项的值被改变, _ABC_SyncData);
			return base.OnBuffPreRemove();
		}
		private void _ABC_SyncData(DS_ActionBusArguGroup ds)
		{
			if (ds.ActionType != ActionBus_ActionTypeEnum.L_DataEntry_OnDataEntryValueChanged_数据项的值被改变)
			{
				return;
			}
			if ((RP_DataEntry_EnumType)ds.IntArgu1.Value != RP_DataEntry_EnumType.SPMax_最大SP)
			{
				return;
			}
			var entry = ds.ObjectArgu1 as Float_RPDataEntry;
			float beforeChange = ds.FloatArgu2.Value;
			float beforePartial = _entry_CurrentSP.CurrentValue / beforeChange;
			beforePartial = UnityEngine.Mathf.Clamp01(beforePartial);
			float afterChange = ds.FloatArgu1.Value;

			float afterValue = beforePartial * afterChange;

			_entry_CurrentSP.SetUpperBound(_entry_MaxSP.CurrentValue);
			if (float.IsNaN(afterValue))
			{
				_entry_CurrentSP.ResetDataToValue(_entry_MaxSP.CurrentValue);
			}
			else
			{
				_entry_CurrentSP.ResetDataToValue(afterValue);
			}
		}
	}
}