...
{
	[Serializable]
	public class Buff_EP_奥能盾卫_ArcaneShielder : BaseRPBuff, I_BuffCanAsEquipmentPerk
	{
		...
		public struct BuffData
		{
			...
			public int Level;
			[SerializeField, LabelText("水火风土伤害减少百分比，于抵抗计算后,过盾计算前"), SuffixLabel("%")]
		...
		private void _ABC_CheckIfNeedReduceDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (dar.DamageType != DamageTypeEnum.AoNengShui_奥能水 && dar.DamageType != DamageTypeEnum.AoNengHuo_奥能火 &&
			    dar.DamageType != DamageTypeEnum.AoNengFeng_奥能风 && dar.DamageType != DamageTypeEnum.AoNengTu_奥能土)
			{
				return;
			}
			dar.CP_DamageAmount_DPart.AddonPart -= (CurrentDamageReduce / 100f);
		}
