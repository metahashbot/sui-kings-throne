...
{
	[Serializable]
	public class Buff_EP_钢铁护卫_SteelGuard : BaseRPBuff, I_BuffCanAsEquipmentPerk
	{
		[Serializable]
		public struct BuffData
		{
			...
			public int Level;
			[SerializeField, LabelText("防御增加百分比"), SuffixLabel("%")]
		...
		{
			_entry_Defense = Parent_SelfBelongToObject
				.ReceiveBuff_GetFloatDataEntry(RP_DataEntry_EnumType.Defense_防御力);
			...
			switch (blp)
			{
				case BLP_作为Perk的等级_ActAsPerkLevel perkLevel:
					...
					_selfModify = Float_ModifyEntry_RPDataEntry.GetNewFromPool(buffData.DefensePartial ,
						...
						this);
					_entry_Defense.AddDataEntryModifier(_selfModify);
					break;
			}
		}
		...
		{
			_entry_Defense.RemoveEntryModifier(_selfModify);
