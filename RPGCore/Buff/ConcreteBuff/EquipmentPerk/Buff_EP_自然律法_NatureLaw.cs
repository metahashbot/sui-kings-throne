...
{
	[Serializable]
	public class Buff_EP_自然律法_NatureLaw : BaseRPBuff, I_BuffCanAsEquipmentPerk
	{
		[Serializable]
		public struct BuffData
		{
			...
			public int Level;
			[SerializeField, LabelText("恢复HP最大的百分比,填30就是回30%"), SuffixLabel("%")]
		...
		private void _ABC_CheckIfRestoreSP_OnSkillNormalReleaseEnd(DS_ActionBusArguGroup ds)
		{
			...
			{
				var restore = _entry_MaxHP.CurrentValue * (CurrentRestorePartial / 100f);
				_entry_CurrentHP.AddDataEntryModifier(Float_ModifyEntry_RPDataEntry.GetNewFromPool(restore,
