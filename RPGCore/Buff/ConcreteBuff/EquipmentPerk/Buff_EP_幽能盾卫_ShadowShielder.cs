...
{
	[Serializable]
	public class Buff_EP_幽能盾卫_ShadowShielder : BaseRPBuff, I_BuffCanAsEquipmentPerk
	{
		...
		public struct BuffData
		{
			...
			public int Level;
			[SerializeField, LabelText("幽能伤害减少百分比，于抵抗计算后,过盾计算前"), SuffixLabel("%")]
		...
		private void _ABC_CheckIfNeedReduceDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (dar.DamageType != DamageTypeEnum.YouNengXingHong_幽能猩红 &&
			    dar.DamageType != DamageTypeEnum.YouNengHunDun_幽能混沌 &&
			    dar.DamageType != DamageTypeEnum.YouNengAnYing_幽能暗影)
			{
				return;
			}
