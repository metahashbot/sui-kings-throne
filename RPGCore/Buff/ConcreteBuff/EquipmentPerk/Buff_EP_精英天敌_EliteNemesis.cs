...
{
	[Serializable]
	public class Buff_EP_精英天敌_EliteNemesis: BaseRPBuff, I_BuffCanAsEquipmentPerk
	{
		...
		public struct BuffData
		{
			...
			public int Level;
			[SerializeField, LabelText("对精英伤害的修正百分比"), SuffixLabel("%")]
		...
		private void _ABC_CheckIfNeedAddDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (dar.Receiver.ReceiveDamage_CheckTargetBuff(RolePlay_BuffTypeEnum._EnemyTag_EliteEnemy_精英敌人) !=
		...
		private void _ABC_CheckIfNeedReduceDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (dar.Caster.ApplyDamage_CheckTargetBuff(RolePlay_BuffTypeEnum._EnemyTag_EliteEnemy_精英敌人) !=
			    BuffAvailableType.NotExist)
			{
				dar.CP_DamageAmount_DPart.MultiplyPart -= ( CurrentDamageOffset / 100f);
			}
