...
{
	[Serializable]
	public class Buff_EP_风息_WindBreath : BaseRPBuff
	{
		[Serializable]
		public struct BuffData
		{
			...
			public int Level;
			[SerializeField, LabelText("风系技能伤害增加百分比"), SuffixLabel("%")]
			public float DamageBonus_WindSkill;
		}
		...
		public override DS_ActionBusArguGroup OnBuffInitialized(List<BaseBuffLogicPassingComponent> blps)
		{
			Parent_SelfBelongToObject.ReceiveBuff_GetRelatedActionBus()
				.RegisterAction(ActionBus_ActionTypeEnum.L_DamageProcess_SkillPower_技能威力计算, _ABC_CheckIfNeedAddDamage);
		...
		private void _ABC_CheckIfNeedAddDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (!dar.DamageFromTypeFlags.HasFlag(DamageFromTypeFlag.PlayerSkillAttack_玩家技能伤害) ||
			    dar.DamageType != DamageTypeEnum.AoNengFeng_奥能风)
			{
				return;
			}
		...
		protected override void ProcessPerBLP(BaseBuffLogicPassingComponent blp)
		{
			...
			switch (blp)
			{
				case BLP_作为Perk的等级_ActAsPerkLevel perkLevel:
					...
					CurrentDamageBonus = buffData.DamageBonus_WindSkill;
					break;
			}
		}
