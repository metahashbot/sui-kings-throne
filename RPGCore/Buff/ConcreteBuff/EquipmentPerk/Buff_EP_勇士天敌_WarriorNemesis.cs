...
{
	[Serializable]
	public class Buff_EP_勇士天敌_WarriorNemesis : BaseRPBuff, I_BuffCanAsEquipmentPerk
	{
		...
		public struct BuffData
		{
			...
			public int Level;
			[SerializeField, LabelText("对勇士伤害的修正百分比"), SuffixLabel("%")]
		...
		private void _ABC_CheckIfNeedAddDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (dar.Receiver.ReceiveDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_CommonWild_基本狂化) ==
			    BuffAvailableType.NotExist &&
			    dar.Receiver.ReceiveDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_SingleBattleWild_单体战斗狂化) ==
			    BuffAvailableType.NotExist &&
			    dar.Receiver.ReceiveDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_CommonSplit_通用分裂) ==
			    BuffAvailableType.NotExist &&
			    dar.Receiver.ReceiveDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_CommonShieldBuilding_基本盾构) ==
			    BuffAvailableType.NotExist &&
			    dar.Receiver.ReceiveDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_ShockWaveShieldBuilding_冲击波盾构) ==
			    BuffAvailableType.NotExist)
			{
				return;
			}
		...
		private void _ABC_CheckIfNeedReduceDamage(DS_ActionBusArguGroup ds)
		{
			...
			if (dar.Caster.ApplyDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_CommonWild_基本狂化) ==
			    BuffAvailableType.NotExist &&
			    dar.Caster.ApplyDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_SingleBattleWild_单体战斗狂化) ==
			    BuffAvailableType.NotExist &&
			    dar.Caster.ApplyDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_CommonSplit_通用分裂) ==
			    BuffAvailableType.NotExist &&
			    dar.Caster.ApplyDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_CommonShieldBuilding_基本盾构) ==
			    BuffAvailableType.NotExist &&
			    dar.Caster.ApplyDamage_CheckTargetBuff(RolePlay_BuffTypeEnum.Enemy_ShockWaveShieldBuilding_冲击波盾构) ==
			    BuffAvailableType.NotExist)
			{
				return;
			}
