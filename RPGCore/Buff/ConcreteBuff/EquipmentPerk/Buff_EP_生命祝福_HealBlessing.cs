...
{
	[Serializable]
	public class Buff_EP_生命祝福_HealBlessing : BaseRPBuff , I_BuffCanAsEquipmentPerk
	{
		[Serializable]
		public struct BuffData
		{
			...
			public int Level;
			[SerializeField,LabelText("最大生命值增加百分比"),SuffixLabel("%")]
			public float HPPartial;
		}
		...
		protected override void ProcessPerBLP(BaseBuffLogicPassingComponent blp)
		{
			...
			switch (blp)
			{
				case BLP_作为Perk的等级_ActAsPerkLevel perkLevel:
					...
					_selfModify = Float_ModifyEntry_RPDataEntry.GetNewFromPool(buffData.HPPartial,
						RPDM_DataEntry_ModifyFrom.FromEquipment_来自装备,
						ModifyEntry_CalculatePosition.FrontMul,
						this);
					_entry_MaxHP.AddDataEntryModifier(_selfModify);
					break;
			}
		}
		...
		protected override void ClearAndUnload()
		{
			if (_selfModify != null)
			{
				_entry_MaxHP.RemoveEntryModifier(_selfModify);
