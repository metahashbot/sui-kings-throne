...
namespace RPGCore.Buff.ConcreteBuff.Utility
{
	...
	[Serializable]
	public class Buff_弱霸体_WeakStoic : BaseRPBuff  
	{
		...
		private string _outlinePresetName_Stoic = "弱霸体描边";
		...
		protected override void ProcessPerBLP(BaseBuffLogicPassingComponent blp)
		{
			...
			switch (blp)
			{
				case BLP_霸体施加信息_StoicApplyInfoBLP stoicApplyInfoBlp:
					...
					if (_stoicApplyInfoList.Count >= 1)
					{
		...
		private void UpdateAvailableTimeByInfoList()
		{
			...
			if( maxDuration < 0f)
			{
				Parent_SelfBelongToObject.ReceiveBuff_RemoveTargetBuff(RolePlay_BuffTypeEnum.WeakStoic_弱霸体);
				return;
			}
			else
			{
