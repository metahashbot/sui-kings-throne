...
{
	[Serializable]
	public class PAEC_播放音效_PlayAudio : BasePlayerAnimationEventCallback
	{
		...
		[ListDrawerSettings( OnBeginListElementGUI =  "RandomAudioIDsList_Begin", OnEndListElementGUI = "RandomAudioIDsList_End")]
#endif
		...
		{
			GeneralAudioManager.Instance.PlayAudioBy(AudioID,
				PlayOptions,
				NoPositionInfo ? null : behaviour.transform.position);
		...
		{
			var clip = GlobalConfigurationAssetHolderHelper.GetGCAHH().FE_GeneralAudioResource
				.GetAudioClipByID(AudioID);
			if (clip == null)
			{
		...
		{ 
			return $"{GetBaseCustomName()} 播放音效：_{(AudioID )}_";
		}
