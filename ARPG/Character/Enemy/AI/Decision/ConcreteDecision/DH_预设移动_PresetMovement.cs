...
{
	[Serializable]
	public class DH_预设移动_PresetMovement : BaseDecisionHandler
	{
		...
		protected GameObject _TargetPoint_FromPosition;

		protected GameObject _TargetPoint_TargetPosition;
		...
		public override DS_ActionBusArguGroup OnDecisionBeforeStartExecution(bool withSideEffect = true)
		{
			...
			{
				DBug.LogError($" {_selfRelatedBehaviourRef.name} 没有关联的生成点信息，无法执行预设移动");
			}
			if (spawnPointInfo.PresetType != EnemySpawnPoint_PresetTypeEnum.FromAndTo_从这里到那里)
			{
				DBug.LogError($" {_selfRelatedBehaviourRef.name} 的生成点信息不是从这里到那里，无法执行预设移动");
			}
			...
			_movementTargetPosition = spawnPointInfo.MatchingGameObjectList[1].transform.position;
			// _movementTargetPosition.y = 0f;
			...
			Vector3 movementDir = MathExtend.NormalizedAsY0(_movementTargetPosition, selfPos);
